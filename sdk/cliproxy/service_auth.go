	...
	for _, update := range updates {
		...
		case watcher.AuthUpdateActionAdd, watcher.AuthUpdateActionModify:
			...
			if versionedStore && authUpdateUsesAuthoritativeStore(update.Auth) {
				...
				if _, errReload := s.reloadAuthRuntimeByID(registrationCtx, update.Auth.ID); errReload != nil {
...
func (s *Service) reconcileAuthRuntime(ctx context.Context) error {
	...
	for id, current := range after {
		...
		if latest, exists := s.coreManager.GetByID(id); !exists || latest == nil || latest.Disabled {
			GlobalModelRegistry().UnregisterClient(id)
		} else if !strings.EqualFold(strings.TrimSpace(latest.Provider), strings.TrimSpace(current.Provider)) {
			s.refreshModelRegistrationForAuth(latest)
		}
		changed = true
	}
...
func (s *Service) prepareCoreAuthForModelRegistration(ctx context.Context, auth *coreauth.Auth) *coreauth.Auth {
	...
	if existing, ok := s.coreManager.GetByID(authID); ok {
		if isStaleCoreAuth(existing, auth) {
			log.Debugf("skipping stale auth update for %s: incoming gen=%d, existing gen=%d", authID, auth.Generation, existing.Generation)
			return existing
		}
		...
		op = "update"
		res, err = s.coreManager.Update(ctx, auth)
	} else {
		res, err = s.coreManager.Register(ctx, auth)
	}
	if err != nil {
		log.Errorf("failed to %s auth %s: %v", op, authID, err)
