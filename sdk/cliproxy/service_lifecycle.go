...
func (s *Service) Run(ctx context.Context) error {
	...
	if s.cfg != nil && s.cfg.Cluster.Enabled && s.coreManager != nil {
		if err := s.bootstrapCluster(ctx); err != nil {
			log.WithError(err).Error("cluster bootstrap failed; continuing in single-instance mode")
		}
	}
	...
	if s.coreManager != nil {
		s.codexWeeklyAutomation = codexweekly.NewAutomation(s.coreManager, func() *config.Config { s.cfgMu.RLock(); defer s.cfgMu.RUnlock(); return s.cfg })
		s.codexHourlyAutomation = codexhourly.NewAutomation(s.coreManager, func() *config.Config { s.cfgMu.RLock(); defer s.cfgMu.RUnlock(); return s.cfg })
		if s.server != nil {
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		if s.clusterRegistrar != nil {
			...
			_ = s.clusterRegistrar.Drain(drainCtx)
			cancel()
		}
		if s.usageSink != nil {
