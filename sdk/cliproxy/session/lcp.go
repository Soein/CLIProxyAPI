...
func appendInteractionValue(turns *[]CanonicalTurn, value gjson.Result, inheritedRole string) bool {
	return appendInteractionValueAtDepth(turns, value, inheritedRole, 0)
}

func appendInteractionValueAtDepth(turns *[]CanonicalTurn, value gjson.Result, inheritedRole string, depth int) bool {
	if !value.Exists() {
		return true
	}
	if !hasCanonicalTurnCapacity(turns) {
	...
	if depth >= maxCanonicalJSONDepth {
		appendTurn(turns, defaultInteractionRole(inheritedRole), []CanonicalPart{canonicalDepthLimitPart(value)})
		return hasCanonicalTurnCapacity(turns)
	}
	if value.Type == gjson.JSON && value.IsArray() {
		value.ForEach(func(_, child gjson.Result) bool {
			return appendInteractionValueAtDepth(turns, child, inheritedRole, depth+1)
		})
		return hasCanonicalTurnCapacity(turns)
	}
	if value.Type != gjson.JSON {
		appendTurn(turns, defaultInteractionRole(inheritedRole), canonicalPartsFromJSONAtDepth(value, depth+1))
		return hasCanonicalTurnCapacity(turns)
	}
	if steps := value.Get("steps"); steps.IsArray() {
		...
		steps.ForEach(func(_, child gjson.Result) bool {
			return appendInteractionValueAtDepth(turns, child, role, depth+1)
		})
		return hasCanonicalTurnCapacity(turns)
	}
	...
	parts := canonicalPartsFromJSONAtDepth(content, depth+1)
	if len(parts) == 0 && !content.Exists() {
		parts = canonicalPartsFromJSONAtDepth(value, depth+1)
	}
	appendTurn(turns, role, parts)
	return hasCanonicalTurnCapacity(turns)
}
...
func canonicalPartsFromJSON(value gjson.Result) []CanonicalPart {
	return canonicalPartsFromJSONAtDepth(value, 0)
}

func canonicalPartsFromJSONAtDepth(value gjson.Result, depth int) []CanonicalPart {
	if !value.Exists() {
		return nil
	}
	if depth >= maxCanonicalJSONDepth {
		return []CanonicalPart{canonicalDepthLimitPart(value)}
	}
	switch value.Type {
	case gjson.String:
	...
	case gjson.JSON:
		...
		if value.IsArray() {
			...
			value.ForEach(func(_, child gjson.Result) bool {
				...
				childParts := canonicalPartsFromJSONAtDepth(child, depth+1)
		...
		if content := value.Get("content"); content.Exists() {
			return canonicalPartsFromJSONAtDepth(content, depth+1)
		}
		if parts := value.Get("parts"); parts.Exists() {
			return canonicalPartsFromJSONAtDepth(parts, depth+1)
		}
		typ := strings.ToLower(strings.TrimSpace(value.Get("type").String()))
...
func canonicalJSONPart(kind string, value gjson.Result) CanonicalPart {
	...
	if jsonDepthExceedsLimit(raw) {
		return canonicalDepthLimitPart(value)
	}
	var decoded any
...
func canonicalDepthLimitPart(value gjson.Result) CanonicalPart {
	raw := value.Raw
	if len(raw) > largePartThreshold {
		return CanonicalPart{
			Kind:         "json:depth-limit",
			Value:        sparseSample(raw, sparseFingerprintBytes),
			Digest:       computeHexSHA256(raw),
			OriginalSize: len(raw),
			Sampled:      true,
	...
	return CanonicalPart{Kind: "json:depth-limit", Value: raw, OriginalSize: len(raw)}
}

func jsonDepthExceedsLimit(raw string) bool {
	depth := 0
	inString := false
	escaped := false
