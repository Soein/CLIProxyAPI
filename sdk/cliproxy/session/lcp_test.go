...
func TestExtractCanonicalTurnsBoundsNestedPartDepth(t *testing.T) {
	t.Parallel()

	const depth = 72
	content := strings.Repeat("[", depth) + `"deep"` + strings.Repeat("]", depth)
	body := `{"messages":[{"role":"user","content":` + content + `}]}`

	turns := ExtractCanonicalTurns(sdktranslator.FormatOpenAI, []byte(body))
	if len(turns) != 1 || len(turns[0].Parts) != 1 {
		t.Fatalf("ExtractCanonicalTurns() = %#v, want one bounded turn part", turns)
	}
	if kind := turns[0].Parts[0].Kind; kind != "json:depth-limit" {
		t.Fatalf("nested part kind = %q, want json:depth-limit", kind)
	}
}

func TestExtractCanonicalTurnsBoundsOpaqueJSONDepth(t *testing.T) {
	t.Parallel()
	...
	if kind := turns[0].Parts[0].Kind; kind != "json:depth-limit" {
		t.Fatalf("opaque nested JSON kind = %q, want json:depth-limit", kind)
	}
}

func TestExtractCanonicalTurnsBoundsNestedInteractionDepth(t *testing.T) {
	t.Parallel()
	...
	turns := ExtractCanonicalTurns(sdktranslator.FormatInteractions, []byte(body))
	if len(turns) != 1 || len(turns[0].Parts) != 1 {
		t.Fatalf("ExtractCanonicalTurns() = %#v, want one bounded turn part", turns)
	}
	if kind := turns[0].Parts[0].Kind; kind != "json:depth-limit" {
		t.Fatalf("nested interaction kind = %q, want json:depth-limit", kind)
	}
}
