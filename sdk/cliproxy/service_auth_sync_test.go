...
func TestRuntimeAuthSyncHook_SynchronousModelAndSchedulerRestoration(t *testing.T) {
	...
	disabledAuth := currentAuth.Clone()
	...
	updatedDisabled, errUpdate := manager.Update(context.Background(), disabledAuth)
	if errUpdate != nil {
	...
	updatedEnabled, errUpdate := manager.Update(context.Background(), enabledAuth)
	if errUpdate != nil {
	...
	quotaAuth := currentAuth.Clone()
	quotaAuth.Quota = coreauth.QuotaState{
		Exceeded:      true,
		Reason:        "credential_quota",
		NextRecoverAt: time.Now().Add(10 * time.Minute),
	}
	updatedQuota, errUpdate := manager.Update(context.Background(), quotaAuth)
	if errUpdate != nil {
		t.Fatalf("manager update quota failed: %v", errUpdate)
	}
	if err := hook(context.Background(), updatedQuota); err != nil {
...
func TestRuntimeAuthSync_FileSnapshotEnqueuedThenRequestExecutionThenConsume(t *testing.T) {
	...
	// True synthesizer builds fresh Auth from disk without private runtime fields.
	raw, errMarshal := json.Marshal(registeredAuth)
	if errMarshal != nil {
		t.Fatalf("marshal registered auth: %v", errMarshal)
	}
	var fileSnapshotAuth coreauth.Auth
	if errUnmarshal := json.Unmarshal(raw, &fileSnapshotAuth); errUnmarshal != nil {
		t.Fatalf("unmarshal file snapshot auth: %v", errUnmarshal)
	}
...
func TestPrepareCoreAuthForModelRegistration_InvalidWeightDoesNotPanic(t *testing.T) {
	reg := internalregistry.GetGlobalRegistry()
	authID := "invalid-weight-test-auth"
	reg.UnregisterClient(authID)
	t.Cleanup(func() {
		reg.UnregisterClient(authID)
	})
	...
	registered, errRegister := manager.Register(context.Background(), validAuth)
	if errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	}
	...
	// Register models for the valid credential
	if res := service.prepareCoreAuthForModelRegistration(context.Background(), registered); res == nil {
		t.Fatal("expected prepareCoreAuthForModelRegistration to succeed for valid auth")
	}
	service.completeModelRegistrationForAuth(context.Background(), registered)

	if models := reg.GetModelsForClient(authID); len(models) == 0 {
		t.Fatal("expected models registered for valid auth, got none")
	}

	// Update with invalid weight: must not panic and must preserve existing credential state
	invalidWeightUpdate := registered.Clone()
	invalidWeightUpdate.Attributes = map[string]string{
		"plan_type":              "pro",
		coreauth.AttributeWeight: "not-a-valid-weight",
	}

	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("prepareCoreAuthForModelRegistration panicked on invalid weight: %v", r)
		}
	}()

	res := service.prepareCoreAuthForModelRegistration(context.Background(), invalidWeightUpdate)
	if res == nil {
		t.Fatal("expected fallback to existing auth snapshot on update failure, got nil")
	}
	if res.ID != authID {
		t.Fatalf("expected auth ID %q, got %q", authID, res.ID)
	}

	// Confirm manager retains previous valid credential without invalid weight
	current, exists := manager.GetByID(authID)
	if !exists {
		t.Fatal("expected auth to still exist in manager")
	}
	if current.Attributes[coreauth.AttributeWeight] == "not-a-valid-weight" {
		t.Fatal("invalid weight was unexpectedly persisted into manager")
	}

	// Confirm models remain registered in global registry
	if models := reg.GetModelsForClient(authID); len(models) == 0 {
		t.Fatal("expected models to remain registered after failed invalid weight update")
	}
}

func TestPrepareCoreAuthForModelRegistration_StaleRegistrationEpochDoesNotPanic(t *testing.T) {
	reg := internalregistry.GetGlobalRegistry()
	authID := "stale-epoch-test-auth"
	reg.UnregisterClient(authID)
	t.Cleanup(func() {
		reg.UnregisterClient(authID)
	})
	...
	// Bump epoch via an update
	update1 := registered.Clone()
	update1.Attributes["custom_key"] = "val1"
	updated1, errUpdate := manager.Update(context.Background(), update1)
	if errUpdate != nil {
		t.Fatalf("manager update failed: %v", errUpdate)
	}
	...
	service.completeModelRegistrationForAuth(context.Background(), updated1)
	if models := reg.GetModelsForClient(authID); len(models) == 0 {
		t.Fatal("expected models registered for valid auth, got none")
	}

	// Attempt update with a stale registration epoch
	staleAuth := updated1.Clone()
	staleAuth.RegistrationEpoch = 1
	if staleAuth.RegistrationEpoch >= updated1.RegistrationEpoch {
		staleAuth.RegistrationEpoch = updated1.RegistrationEpoch - 1
	}

	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("prepareCoreAuthForModelRegistration panicked on stale epoch: %v", r)
		}
	}()

	res := service.prepareCoreAuthForModelRegistration(context.Background(), staleAuth)
	if res == nil {
		t.Fatal("expected fallback to existing auth snapshot on stale epoch, got nil")
	}
	if res.ID != authID {
		t.Fatalf("expected auth ID %q, got %q", authID, res.ID)
	}

	// Confirm manager retains the latest valid snapshot
	current, exists := manager.GetByID(authID)
	if !exists {
		t.Fatal("expected auth to still exist in manager")
	}
	if current.RegistrationEpoch != updated1.RegistrationEpoch {
		t.Fatalf("expected epoch %d, got %d", updated1.RegistrationEpoch, current.RegistrationEpoch)
	}

	// Confirm models remain registered
	if models := reg.GetModelsForClient(authID); len(models) == 0 {
		t.Fatal("expected models to remain registered after failed stale epoch update")
	}
}

func TestPrepareCoreAuthForModelRegistration_NewAuthInvalidWeightDoesNotPanic(t *testing.T) {
	reg := internalregistry.GetGlobalRegistry()
	authID := "new-invalid-weight-auth"
	reg.UnregisterClient(authID)
	t.Cleanup(func() {
		reg.UnregisterClient(authID)
	})
	...
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("prepareCoreAuthForModelRegistration panicked on new auth invalid weight: %v", r)
		}
	}()

	res := service.prepareCoreAuthForModelRegistration(context.Background(), invalidAuth)
	if res != nil {
		t.Fatalf("expected nil for failed new registration, got %#v", res)
	}

	if _, exists := manager.GetByID(authID); exists {
		t.Fatal("manager unexpectedly registered auth with invalid weight")
	}
	if models := reg.GetModelsForClient(authID); len(models) != 0 {
		t.Fatalf("expected 0 models for unregistered auth, got %d", len(models))
	}
}
