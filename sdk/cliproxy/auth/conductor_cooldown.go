...
func (m *Manager) markResult(ctx context.Context, result Result, affinityState resultSessionAffinity, affinityIntermediate bool, routeModel string, skipQuotaObservation bool) {
	if result.AuthID == "" {
		return
	}
	if ctx == nil {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if modelKey == "" && strings.TrimSpace(routeModel) != "" {
			if m != nil {
...
func (m *Manager) recordExecutionResultWithAffinity(ctx context.Context, result Result, auth *Auth, ephemeral bool, affinityState resultSessionAffinity, affinityIntermediate bool, routeModel string, skipQuotaObservation bool) {
	if !ephemeral {
		m.markResult(ctx, result, affinityState, affinityIntermediate, routeModel, skipQuotaObservation)
		return
	}
...
func (m *Manager) reportHomeResult(ctx context.Context, result Result, auth *Auth) {
	...
	affinityState := sessionAffinityResultForRequest(ctx, result.Provider, result.Model, result.Options)
	m.updateSessionAffinity(resultForSessionAffinity(result, affinityState), affinityState)
}
