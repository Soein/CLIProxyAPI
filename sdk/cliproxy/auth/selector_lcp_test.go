...
func (s *blockingFirstLCPSelector) Pick(_ context.Context, _, _ string, _ cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	s.mu.Lock()
	s.calls++
	call := s.calls
	if call == 1 {
...
func (s *blockingFirstLCPSelector) callCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.calls
}

func TestSessionAffinitySelectorLCPColdBindingHonorsPreferredExecutionAuth(t *testing.T) {
	t.Parallel()
	...
	opts := cliproxyexecutor.Options{
		...
		OriginalRequest: []byte(`{"messages":[{"role":"user","content":"prefer the active websocket"}]}`),
		Metadata: map[string]any{
			cliproxyexecutor.CallerScopeMetadataKey: "caller-preferred",
		},
	}

	selected, errPick := selector.pickForExecution(context.Background(), "openai", "model", opts, auths, func(string) int { return 0 }, "auth-b")
	if errPick != nil {
		t.Fatalf("pickForExecution() error = %v", errPick)
	}
	if selected.ID != "auth-b" {
		t.Fatalf("pickForExecution() = %q, want preferred auth-b", selected.ID)
	}
}

func TestSessionAffinitySelectorLCPSerializesConcurrentColdBinding(t *testing.T) {
	t.Parallel()

	fallback := &blockingFirstLCPSelector{
	...
	newOptions := func() cliproxyexecutor.Options {
		return cliproxyexecutor.Options{
			SourceFormat:    sdktranslator.FormatOpenAI,
			OriginalRequest: []byte(`{"messages":[{"role":"user","content":"same concurrent conversation"}]}`),
			Metadata: map[string]any{
				cliproxyexecutor.CallerScopeMetadataKey: "caller-concurrent",
			},
		}
	}
	...
	firstDone := make(chan pickResult, 1)
	secondDone := make(chan pickResult, 1)
	go func() {
		auth, errPick := selector.Pick(context.Background(), "openai", "model", newOptions(), auths)
		firstDone <- pickResult{auth: auth, err: errPick}
	}()
	<-fallback.firstEntered
	go func() {
		auth, errPick := selector.Pick(context.Background(), "openai", "model", newOptions(), auths)
		secondDone <- pickResult{auth: auth, err: errPick}
	}()

	var second pickResult
	select {
	...
	if first.err != nil || second.err != nil {
		t.Fatalf("concurrent Pick() errors = (%v, %v)", first.err, second.err)
	}
	if first.auth.ID != second.auth.ID {
		t.Fatalf("concurrent cold bindings selected different auths: first=%q second=%q", first.auth.ID, second.auth.ID)
	}
	if calls := fallback.callCount(); calls != 1 {
		t.Fatalf("fallback Pick() calls = %d, want 1 for one cold LCP binding", calls)
	}
}

func TestSessionAffinitySelectorLCPColdBindingHonorsLeastInflightAuth(t *testing.T) {
	t.Parallel()
	...
	opts := cliproxyexecutor.Options{
		SourceFormat:    sdktranslator.FormatOpenAI,
		OriginalRequest: []byte(`{"messages":[{"role":"user","content":"choose the least loaded credential"}]}`),
		Metadata: map[string]any{
			cliproxyexecutor.CallerScopeMetadataKey: "caller-inflight",
		},
	}
	inflight := func(authID string) int {
		if authID == "auth-a" {
			return 3
		}
		return 0
	...
	selected, errPick := selector.pickForExecution(context.Background(), "openai", "model", opts, auths, inflight, "")
	if errPick != nil {
		t.Fatalf("pickForExecution() error = %v", errPick)
	}
	if selected.ID != "auth-b" {
		t.Fatalf("pickForExecution() = %q, want least-inflight auth-b", selected.ID)
	}
}

func TestSessionAffinitySelectorLCPRecordsNormalizedOutcomes(t *testing.T) {
	t.Parallel()
	...
	pick := func(opts cliproxyexecutor.Options) coreusage.AffinityOutcome {
		t.Helper()
		selection := coreusage.BeginAuthSelection(ctx)
		if _, errPick := selector.Pick(ctx, "openai", "model", opts, auths); errPick != nil {
			t.Fatalf("Pick() error = %v", errPick)
		}
		selection.Complete()
		return tracker.BeginAttempt().AffinityOutcome
	}

	first := cliproxyexecutor.Options{
		SourceFormat:    sdktranslator.FormatOpenAI,
		OriginalRequest: []byte(`{"messages":[{"role":"user","content":"first"}]}`),
		Metadata: map[string]any{
			cliproxyexecutor.CallerScopeMetadataKey: "caller-outcomes",
		},
	}
	if outcome := pick(first); outcome != coreusage.AffinityOutcomeMiss {
		t.Fatalf("first LCP outcome = %q, want miss", outcome)
	}

	grown := cliproxyexecutor.Options{
		SourceFormat:    sdktranslator.FormatOpenAI,
		OriginalRequest: []byte(`{"messages":[{"role":"user","content":"first"},{"role":"assistant","content":"answer"},{"role":"user","content":"continue"}]}`),
		Metadata: map[string]any{
			cliproxyexecutor.CallerScopeMetadataKey: "caller-outcomes",
		},
	}
	if outcome := pick(grown); outcome != coreusage.AffinityOutcomeHit {
		t.Fatalf("grown LCP outcome = %q, want hit", outcome)
	}
}
