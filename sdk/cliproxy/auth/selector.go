...
func (s *SessionAffinitySelector) pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, inflight func(string) int, preferredAuthID string) (*Auth, error) {
	...
	if explicitID == "" {
		if auth, handled, errLCP := s.pickLCP(ctx, provider, model, opts, auths, entry, inflight, preferredAuthID); handled || errLCP != nil {
			return auth, errLCP
		}
	...
	if explicitID != "" {
		// Validate hierarchy before caller scoping makes unrelated aliases share a prefix.
		if isHierarchyParent(explicitID, explicitFallbackID) && fallbackID != "" && fallbackID != primaryID {
	...
	isSubagent := !isFork && isSubagentSession(explicitID, explicitFallbackID)
...
func (s *SessionAffinitySelector) pickLCP(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, entry *log.Entry, inflight func(string) int, preferredAuthID string) (*Auth, bool, error) {
	...
	lockKey := namespace + "\x00" + strings.Join(fingerprints[:minPrefixLength], "\x00")
	unlock := s.lockSessionKeys(lockKey, "")
	defer unlock()
	...
	match, matched := s.matcher.MatchFingerprints(namespace, fingerprints, minPrefixLength)
	if matched {
		for _, auth := range available {
	...
	fallbackAuths := highestPriorityAuths(available)
	auth, errPick := s.pickFallback(ctx, provider, model, opts, fallbackAuths, inflight, preferredAuthID)
	if errPick != nil {
...
func (s *SessionAffinitySelector) LookupAffinity(provider, model, sessionID string, authFilters ...func(authID string) bool) (string, string) {
	...
	for _, candProvider := range providers {
		for _, cand := range candidates {
			...
			key := sessionAffinityCacheKey(candProvider, bounded, modelKey)
...
func (s *SessionAffinitySelector) onResult(res Result, state resultSessionAffinity) {
	...
	if s.cache == nil || state.primaryKey == "" {
		return
	}
	...
	isFork, _ := res.Options.Metadata[cliproxyexecutor.IsForkMetadataKey].(bool)
	updateFallback := state.fallbackKey != "" && !isFork && !isSubagentSession(explicitID, explicitFallbackID)
	if res.Success {
...
func sessionAffinityKeysForRequest(ctx context.Context, provider, model string, opts cliproxyexecutor.Options) sessionAffinityKeys {
	// Identity extraction annotates fork metadata; key lookups must not overwrite picked identities.
...
func (c *sessionAffinityMetadataCloner) clone(value reflect.Value, depth int) (reflect.Value, bool) {
	...
	case reflect.Map:
		...
		for iterator.Next() {
			// Selection callbacks belong to dispatch and capture mutable execution state;
			// they must not be retained by result snapshots or force metadata fallback.
			if depth == 0 && iterator.Key().Kind() == reflect.String {
				switch iterator.Key().String() {
				case cliproxyexecutor.SelectedAuthCallbackMetadataKey, cliproxyexecutor.SelectedAuthIndexCallbackMetadataKey:
					continue
				}
			}
...
func sessionAffinityMetadataFallback(src map[string]any) map[string]any {
	...
	for _, key := range keys {
		...
		case string, int, uint64, bool:
			fallback[key] = value
		case []string:
			fallback[key] = append([]string(nil), value...)
		}
	}
	return fallback
}
...
func extractSessionIDsForProvider(ctx context.Context, provider string, opts cliproxyexecutor.Options) (string, string) {
	...
	explicitPrimary, explicitFallback := extractExplicitSessionIDs(opts.Headers, opts.OriginalRequest, opts.Metadata)
