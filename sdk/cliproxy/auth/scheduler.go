...
func (s *authScheduler) pickSingleWithStrategyAndInflight(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy, inflight func(string) int, preferredAuthID string, eligible func(string) bool) (*Auth, error) {
	...
	if eligible != nil {
		...
		predicate = func(entry *scheduledAuth) bool {
			return requestPredicate(entry) && eligible(entry.auth.ID)
		}
	}
...
func (s *authScheduler) upsertAuthWithoutRebuildLocked(auth *Auth, now time.Time, dirty map[*modelScheduler]struct{}) {
	...
	providerState.upsertAuthWithoutRebuildLocked(meta, now, dirty)
}
...
func (s *authScheduler) upsertAuthResultLocked(auth *Auth, targetModels []string, credentialScoped bool, now time.Time) {
	...
	if s.isStaleScheduledAuth(authID, auth.RegistrationEpoch, auth.Generation, auth.UpdatedAt) ||
		!s.acceptVersionLocked(authID, auth.revision, auth.Disabled || auth.Status == StatusDisabled) {
