...
func (m *Manager) tryRefreshExecutionAuthAfterUnauthorized(ctx context.Context, executor ProviderExecutor, auth *Auth, execErr error, alreadyTried bool, homeDispatch bool) (*Auth, bool, error) {
	if !homeDispatch {
		return m.tryRefreshAfterUnauthorized(ctx, auth, execErr, alreadyTried)
	}
...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	result, err, _ := m.refreshSF.Do(strings.TrimSpace(id), func() (any, error) {
		return m.refreshAuthOnce(ctx, id, failedAccessToken)
	})
	if result == nil {
		return nil, err
	}
	refreshed, _ := result.(*Auth)
	return refreshed, err
}

func (m *Manager) refreshAuthOnce(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	if m == nil {
		return nil, errors.New("auth manager is nil")
	}
	if ctx == nil {
	...
	if locker := m.currentAuthRefreshLocker(); locker != nil {
		release, ok, errLock := locker.TryLock(ctx, id)
		if errLock != nil {
			return nil, errLock
		}
		if !ok {
