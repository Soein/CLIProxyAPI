...
func (m *Manager) syncSchedulerFromSnapshot(auths []*Auth, snapshotWatermark uint64) {
	if m == nil || m.scheduler == nil {
		return
	}
	m.scheduler.rebuild(auths, snapshotWatermark)
...
func (m *Manager) nextAuthRevisionLocked() uint64 { m.authRevision++; return m.authRevision }
func (m *Manager) nextAuthDurableRevisionLocked() uint64 {
	m.authDurableRevision++
	return m.authDurableRevision
}
...
func (m *Manager) syncScheduler() {
	...
	if currentVer == m.syncedVersion.Load() && !m.schedulerNeedsSync() {
		return
	}
	m.checkAndSyncScheduler()
}

func (m *Manager) snapshotAuths() ([]*Auth, uint64) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Manager) checkAndSyncScheduler() bool {
	...
	currentVer := m.currentVersion()
	needsSync := m.schedulerNeedsSync()
	if currentVer == m.syncedVersion.Load() && !needsSync {
		return true
	}
	if !needsSync {
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for {
		...
		if m.PluginSchedulerWantsAcrossPriorities() {
			priorityAuths, errAvailable = m.availableAuthsForRouteModelAcrossPriorities(candidateSnapshots, provider, model, time.Now())
		} else {
			priorityAuths, errAvailable = m.availableAuthsForRouteModel(candidateSnapshots, provider, model, time.Now())
		}
		if errAvailable != nil {
			if spillover() {
				continue
		...
		selected, handled, errPick := m.pickViaPluginScheduler(ctx, pluginScheduler, provider, []string{provider}, model, opts, tried, priorityAuths)
		if errPick != nil {
		...
		if !handled {
			...
			if errSelectorAvailable != nil {
				...
				m.warnLogAuthUnavailable(ctx, []string{provider}, model, opts, tried, errSelectorAvailable)
		...
		if authCopy := m.admitScheduledAuth(selected, provider, model); authCopy != nil {
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for {
		...
		authCopy := m.admitScheduledAuth(selected, provider, model)
		if authCopy == nil {
			if tried == nil {
				tried = make(map[string]struct{})
			}
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	opts.Metadata[cliproxyexecutor.SessionAffinityModelMetadataKey] = selectionArgForSelector(m.Selector(), model)
	auth, executor, provider, errPick := m.pickNextMixedLegacyWithInflight(ctx, providers, model, opts, tried, nil, "")
	if errPick != nil {
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	opts.Metadata[cliproxyexecutor.SessionAffinityProviderMetadataKey] = "mixed"
	opts.Metadata[cliproxyexecutor.SessionAffinityModelMetadataKey] = model
	auth, executor, provider, errPick := m.pickNextMixedWithInflight(ctx, providers, model, opts, tried, nil, "")
	if errPick != nil {
		m.warnLogAuthUnavailable(ctx, providers, model, opts, tried, errPick)
	}
	return auth, executor, provider, errPick
