...
type metadataNormalizationStore struct {
	auths []*Auth
}

func (s *metadataNormalizationStore) List(context.Context) ([]*Auth, error) {
	return s.auths, nil
}

func (*metadataNormalizationStore) Save(context.Context, *Auth) (string, error) {
	return "", nil
}

func (*metadataNormalizationStore) Delete(context.Context, string) error {
	return nil
}
...
func TestManagerLoadNormalizesCredentialMetadataFromCustomStore(t *testing.T) {
	manager := NewManager(&metadataNormalizationStore{auths: []*Auth{{
		ID:       "legacy-store-auth",
		Provider: "codex",
		Metadata: map[string]any{
			"api-key":       "legacy-key",
	...
	loaded, ok := manager.GetByID("legacy-store-auth")
	if !ok || loaded == nil {
		t.Fatal("loaded auth is missing")
	}
	if got := loaded.Metadata["api_key"]; got != "canonical-key" {
		t.Fatalf("loaded api_key = %#v, want canonical-key", got)
	}
	if retry, okRetry := loaded.RequestRetryOverride(); !okRetry || retry != 2 {
		t.Fatalf("RequestRetryOverride() = (%d, %t), want (2, true)", retry, okRetry)
	}
	for _, legacyKey := range []string{"api-key", "request-retry"} {
		if _, exists := loaded.Metadata[legacyKey]; exists {
			t.Fatalf("loaded metadata retained legacy key %q: %#v", legacyKey, loaded.Metadata)
		}
	}
}
