...
func TestSessionAffinityMixedPoolClaudeInheritsWhileAntigravityIsolates(t *testing.T) {
	...
	if bound, ok := selector.cache.Get(sessionAffinityCacheKey("mixed", "claude:mixed-sess-ag", "claude-3-7-sonnet")); !ok || bound != "auth-ag-primary" {
...
func TestSessionAffinityAntigravitySubagentOnResultFailureDoesNotUnbindParent(t *testing.T) {
	...
	parentCacheKey := sessionAffinityCacheKey("mixed", "claude:claude-root-500", "claude-3-7-sonnet")
	if bound, ok := selector.cache.Get(parentCacheKey); !ok || bound != "auth-ag-1" {
