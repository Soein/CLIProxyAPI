...
func markNoForkResult(ctx context.Context, manager *Manager, routeModel string, result Result) {
	affinityState := sessionAffinityResultForRequest(ctx, result.Provider, result.Model, result.Options)
	manager.markResult(ctx, result, affinityState, false, routeModel, false)
}
...
func TestManager_NoForkAlias_MarkResultRouteModelWritesAuthoritativeState(t *testing.T) {
	...
	// Pass routeModel B through the manager's private result path with a 429 failure.
	retryAfter := 20 * time.Minute
	markNoForkResult(context.Background(), manager, routeModel, Result{
		AuthID:   auth.ID,
		Provider: provider,
		Success:  false,
		Error: &Error{
	...
	// Now mark success on routeModel through the same private result path.
	markNoForkResult(context.Background(), manager, routeModel, Result{
		AuthID:   auth.ID,
		Provider: provider,
		Success:  true,
...
func TestManager_NoForkAlias_RealDiskFileStoreRestore_BypassesCoolingAuth(t *testing.T) {
	...
	// Put auth1 into cooldown on targetModel while retaining the logical route model.
	retryAfter := 30 * time.Minute
	markNoForkResult(context.Background(), manager1, routeModel, Result{
		AuthID:   auth1.ID,
		Provider: provider,
		Model:    targetModel,
		Success:  false,
...
func TestManager_NoForkAlias_CancelledContextReconcile_CooldownStoreCleaned(t *testing.T) {
	...
	// Put targetModel into cooldown while retaining the logical route model.
	retryAfter := 30 * time.Minute
	markNoForkResult(context.Background(), manager, routeModel, Result{
		AuthID:   auth.ID,
		Provider: provider,
...
func TestManager_NoForkAlias_MarkResultTargetModelNoSecondaryAliasResolution(t *testing.T) {
	...
	// The execution layer passes authoritative target modelA and logical route modelB separately.
	retryAfter := 20 * time.Minute
	markNoForkResult(context.Background(), manager, modelB, Result{
		AuthID:   auth.ID,
		Provider: provider,
		Model:    modelA,
		Success:  false,
...
func TestManager_NoForkAlias_ChainedAlias_MarkResultTargetA_ReconcilePreservesStateA(t *testing.T) {
	...
	retryAfter := 30 * time.Minute
	markNoForkResult(context.Background(), manager, routeB, Result{
		AuthID:   auth1.ID,
		Provider: provider,
		Model:    targetA,
		Success:  false,
...
func TestManager_NoForkAlias_ResetQuota_ErrorPersistCleansCooldownStore(t *testing.T) {
	...
	if _, errReg := manager.Register(WithSkipPersist(context.Background()), auth); errReg != nil {
		t.Fatalf("register auth: %v", errReg)
	}
...
func TestManager_NoForkAlias_PerModelIsolation_NoAggregationFallbackFlip(t *testing.T) {
	...
	retryAfter := 30 * time.Minute
	markNoForkResult(context.Background(), manager, modelA, Result{
		AuthID:   auth.ID,
		Provider: provider,
		Model:    modelA,
		Success:  false,
...
func TestManager_NoForkAlias_ConcurrentHighLoadStability_EpochAndReconcile(t *testing.T) {
	...
	for i := 0; i < numWorkers/2; i++ {
		...
		go func(workerID int) {
			...
			for j := 0; j < numIters; j++ {
				if j%2 == 0 {
					markNoForkResult(context.Background(), manager, modelName, Result{
						AuthID:   authID,
						Provider: provider,
						Model:    modelName,
						Success:  false,
