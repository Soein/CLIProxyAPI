...
type schedulerCountingRefreshExecutor struct {
	schedulerProviderTestExecutor
	refreshCalls int
}

func (e *schedulerCountingRefreshExecutor) Refresh(_ context.Context, auth *Auth) (*Auth, error) {
	e.refreshCalls++
	return auth, nil
}
...
func TestManager_RefreshAuthLockerErrorFailsClosed(t *testing.T) {
	...
	executor := &schedulerCountingRefreshExecutor{schedulerProviderTestExecutor: schedulerProviderTestExecutor{provider: "codex"}}
...
func TestManager_Persist_SkipPersistAdvancesWatermarkAndBlocksOlderGeneration(t *testing.T) {
	...
	// Simulate a published snapshot still waiting for its lifecycle persistence lock.
	manager.mu.Lock()
	manager.auths[auth.ID] = gen1Auth.Clone()
	manager.markPersistenceInFlightLocked(ctx, gen1Auth)
	manager.mu.Unlock()
	...
	// Lifecycle ordering belongs to the publication wrapper; persist is the
	// lock-free store operation also used by enable rollback while holding the lock.
	if errPersist := manager.persistPublishedIfCurrent(ctx, gen1Auth); errPersist != nil {
	...
	if savedImmediately.Metadata["proxy_url"] != "http://initial.proxy" {
		t.Fatalf("skip-persist watcher state was written by the stale publication: %v", savedImmediately.Metadata["proxy_url"])
	}
	// Let's verify by updating a generation 3 that DOES persist:
	gen3Auth, _ := manager.GetByID(auth.ID)
	...
	if savedFinal.Metadata["access_token"] != "gen3-token" {
		t.Fatalf("access_token in store = %v, want gen3-token", savedFinal.Metadata["access_token"])
	}
}
