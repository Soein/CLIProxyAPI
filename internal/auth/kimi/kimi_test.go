...
func TestKimiMarshalTokenJSONPreservesDomain(t *testing.T) {
	tests := []struct {
		name       string
	...
	}{
		{
			...
			storage:    KimiTokenStorage{},
			wantType:   "kimi",
			wantDomain: KimiDefaultDomain,
			wantURL:    KimiAPIBaseURL,
		},
		{
			name:       "AI domain selects AI type",
			storage:    KimiTokenStorage{Domain: KimiAIDomain},
			wantType:   "kimi-ai",
			wantDomain: KimiAIDomain,
			wantURL:    KimiAIAPIBaseURL,
		},
		{
			name:       "AI type selects AI domain",
			storage:    KimiTokenStorage{Type: "kimi-ai"},
			wantType:   "kimi-ai",
			wantDomain: KimiAIDomain,
			wantURL:    KimiAIAPIBaseURL,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			payload, errMarshal := tt.storage.MarshalTokenJSON()
			if errMarshal != nil {
			...
			if parsed["type"] != tt.wantType || parsed["domain"] != tt.wantDomain || parsed["base_url"] != tt.wantURL {
				t.Fatalf("token metadata = %v, want type=%q domain=%q base_url=%q", parsed, tt.wantType, tt.wantDomain, tt.wantURL)
			}
		})
	}
}
