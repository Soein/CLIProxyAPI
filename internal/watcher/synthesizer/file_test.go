...
func TestSynthesizeAuthFilePreservesRelativeFileName(t *testing.T) {
	authDir := t.TempDir()
	fullPath := filepath.Join(authDir, "nested", "auth.json")
	auths, errSynthesize := SynthesizeAuthFile(&SynthesisContext{
		Config:  &config.Config{},
		AuthDir: authDir,
		Now:     time.Now(),
	}, fullPath, []byte(`{"type":"claude"}`))
	if errSynthesize != nil {
		t.Fatalf("SynthesizeAuthFile() error = %v", errSynthesize)
	}
	if len(auths) != 1 {
	...
	want := filepath.Join("nested", "auth.json")
	if auths[0].ID != want || auths[0].FileName != want {
		t.Fatalf("auth ID = %q, FileName = %q, want both %q", auths[0].ID, auths[0].FileName, want)
	}
}
