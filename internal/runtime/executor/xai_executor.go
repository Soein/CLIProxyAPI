...
func normalizeXAIToolsWithConfig(body []byte, cfg config.XAIConfig) []byte {
	...
	for _, group := range groups {
		...
		updated, errSet := sjson.SetRawBytes(body, group.path, filtered)
		if errSet != nil {
			return original
		}
		body = updated
...
func capXAINormalizedTools(body []byte, tools []xaiNormalizedTool, cfg config.XAIConfig) ([]xaiNormalizedTool, bool) {
	...
	reservedCost := 0
	if !xaiNormalizedToolsContainType(tools, xaiXSearchToolType) {
		// prepareResponsesRequestTo injects native x_search after normalization.
		reservedCost = 1
	}
...
func prioritizeXAITools(tools []xaiNormalizedTool, preferredNamespaces []string, forcedChoice xaiToolSelectionKey) []xaiNormalizedTool {
	...
	// Native x_search is injected for every prepared request and must remain
	// inside the effective upstream tool budget when the request is capped.
