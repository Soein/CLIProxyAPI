...
func TestNewProxyAwareHTTPClientRequestProxyOverridesContextRoundTripper(t *testing.T) {
	t.Parallel()

	roundTripper := &proxyHelperRoundTripper{}
	ctx := context.WithValue(context.Background(), "cliproxy.roundtripper", roundTripper)
	ctx = coreexecutor.WithRequestProxyURL(ctx, "http://request-proxy.example:8081")
	client := NewProxyAwareHTTPClient(ctx, nil, nil, 0)
	if client.Transport == roundTripper {
		t.Fatal("request proxy must replace the injected context RoundTripper")
	}
	transport, ok := client.Transport.(*http.Transport)
	if !ok || transport.Proxy == nil {
		t.Fatalf("transport = %#v, want request proxy", client.Transport)
