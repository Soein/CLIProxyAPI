...
func (e *XAIExecutor) prepareResponsesRequestTo(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, stream bool, to sdktranslator.Format) (*xaiPreparedRequest, error) {
	...
	willInjectXSearch := xaiCfg.InjectXSearch
	...
	body = normalizeXAIToolsWithConfigAndFold(body, xaiCfg, shouldFold)
	...
	if xaiCfg.InjectXSearch && !xaiToolChoiceRequiresImageGenerationOnly(body) {
...
func normalizeXAITools(body []byte) []byte {
	return normalizeXAIToolsWithConfigAndFold(body, config.XAIConfig{}, xaiShouldFoldNamespaceTools(body, false))
}

func normalizeXAIToolsWithConfig(body []byte, cfg config.XAIConfig) []byte {
	return normalizeXAIToolsWithConfigAndFold(body, cfg, xaiShouldFoldNamespaceTools(body, cfg.InjectXSearch))
}

func normalizeXAIToolsWithFold(body []byte, shouldFold bool) []byte {
	return normalizeXAIToolsWithConfigAndFold(body, config.XAIConfig{}, shouldFold)
}

func normalizeXAIToolsWithConfigAndFold(body []byte, cfg config.XAIConfig, shouldFold bool) []byte {
	if !gjson.ValidBytes(body) {
		return body
	}
	...
	collectGroup := func(path string) bool {
		...
		normalized, changed, ok := normalizeXAIToolArray(tools, keepImageGeneration, shouldFold)
		if !ok {
			return false
		}
	...
	retained, capped := capXAINormalizedTools(body, allTools, cfg, shouldFold)
	if capped {
...
func normalizeXAIToolArray(tools gjson.Result, keepImageGeneration, shouldFold bool) ([]xaiNormalizedTool, bool, bool) {
		...
		if toolType == xaiNamespaceToolType {
			...
			if shouldFold {
				if dispatcher := buildXAINamespaceDispatcherTool(tool); len(dispatcher) > 0 {
					namespaceName := strings.TrimSpace(tool.Get("name").String())
					normalized = append(normalized, xaiNormalizedTool{raw: dispatcher, name: namespaceName, namespace: namespaceName})
				}
				continue
			}
...
func capXAINormalizedTools(body []byte, tools []xaiNormalizedTool, cfg config.XAIConfig, shouldFold bool) ([]xaiNormalizedTool, bool) {
	...
	forcedChoice := xaiForcedToolSelectionKeyWithFold(body, shouldFold)
...
func xaiForcedToolSelectionKey(body []byte) xaiToolSelectionKey {
	return xaiForcedToolSelectionKeyWithFold(body, false)
}

func xaiForcedToolSelectionKeyWithFold(body []byte, shouldFold bool) xaiToolSelectionKey {
	choice := gjson.GetBytes(body, "tool_choice")
	if !choice.IsObject() {
	...
	if toolType == xaiFunctionToolType {
		if shouldFold && namespace != "" {
			key.qualifiedName = namespace
			return key
		}
