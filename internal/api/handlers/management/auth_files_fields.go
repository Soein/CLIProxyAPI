...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	locked = false
	h.authStatusMu.Unlock()
	if errHook := h.invokePostAuthPersistHooks(ctx, updated); errHook != nil {
		log.Errorf("post-auth persist hook failed for status update: %v", errHook)
...
func setSourceAuthFileDisabled(path string, disabled bool) error {
	...
	original, updated, errPrepare := prepareSourceAuthFileDisabled(path, disabled)
	if errPrepare != nil {
		return errPrepare
	}
	return writeSourceAuthFileAtomicallyIfUnchanged(path, original, updated)
}
...
func (h *Handler) saveTokenRecord(ctx context.Context, record *coreauth.Auth) (string, error) {
	...
	savedPath, errSave := coreauth.PersistExplicitAuth(coreauth.WithAuthCreationIntent(ctx), store, record)
	if errSave != nil {
