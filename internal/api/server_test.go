...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	wantPriority := codexClientTestMaxTemplatePriority(t) + 100
	if got := codexClientTestPriority(custom["priority"]); got != wantPriority {
		t.Fatalf("custom priority = %v, want %d", custom["priority"], wantPriority)
	}
